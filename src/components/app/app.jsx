...
// Реализовать переиспользуемый мидлвар, который сможет работать с разными 
// сокет-соединениями
...
/*
const initiatState = {
  status: WelsocketStatus.OFFLINE,
  table: [],
  connectionError: '';
}
*/
...
// 1) Создать редьюсер, и в нем обработать экшены для обработчиков событий 
//    onmessage, onclose, onerror и send

// 2) Создать миддлвар: скопировать код из тренажера-из Андрея
// В этот мидлвар урл передавать параметром не надо. Его надо диспатчить
// вместе с экшеном в момент подключения, как payload.
// Параметром в мидлвар передаем только набор экшенов - wsActions.

// Итого: в случае всех событий, которые могут произойти на сокете, 
// мы из мидлвара диспатчим соответствующие экшены в редьюсер. 

// На странице feed надо в юзеффекте сделать подключение к сокету, и
// и там отправить экшены connect и disconnect   (см. скрин)

// А внутри компонента Orders в юзеффекте через useMatch проверяю, открыт ли он со страницы profile/orders
// И если да, то подключаюсь к вебсокету (см. скрин)

// То есть два подключения к вебсокету, и для них - два редьюсера. И для них два набора экшенов 
